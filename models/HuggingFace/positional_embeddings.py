...
class RotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, dim, base=10000, precision=torch.half, learnable=False, max_position_embeddings=30720, scaling_factor=1.0):
        super().__init__()
    ...
    def forward(self, x, seq_dim=1, seq_len=None):
        ...
        if self.max_seq_len_cached is None or (seq_len > self.max_seq_len_cached):
            ...
            if seq_len > self.max_position_embeddings:
                ...
                ntk_inv_freq = 1. / (base ** (torch.arange(0, self.dim, 2).float().to(x.device) / self.dim))
